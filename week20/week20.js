...
function makeTwo() {
	...
	try{
		const e = 1;
		resultTwo.textContent = 'выполнено';
	} catch(error){
...
function makeThree() {
	...
	try {
			throw new Error("вы всё сделали не правильно!")
	} catch (error) {
...
function makeFive() {
	let inputData = -1;
	if (inputData < 0)
		throw new SyntaxError("данные не кореектны")
		else{
...
try{
	...
	document.getElementById('result10').textContent = 'Email-адрес не корректен';
}
}
...
function makeThirteen() {
	...
	try{
		...
		console.log("Доступ запрещен для лиц младше 18 лет")
	}
}
...
function makeFourteen() {
	...
	console.log('Некорректное значение скидки');
}}
...
function makeTwenty() {
	...
	try{
		let x = calculateValue(Math.random())
		resultTwenty.textContent = x;
	}
	catch(error){
...
function makeTwentyOne() {
	...
	try{
		let x = fetchData(Math.random())
		resultTwentyOne.textContent = x;
	}
	catch(error){
		resultTwentyOne.textContent = "Произошла ошибка при получении данных"
	}
	finally{
...
function makeTwentyThree() {
	...
	try{
		const json23 = JSON.parse(json23);
		console.log(json23.name);
	}
	catch(error){
...
function makeTwentyFour() {
	...
	let divisor = 0;
	if (divisor === 0) {
		throw new Error('не делите на ноль пожалуйста')
	}
...
try {
	const result = await simulateAsyncOperation(); // мне кажется сюда нужно
...
function makeTwentyEight() {
    ...
    setTimeout(() => {
        try {
            ...
            console.error('Ошибка на верхнем уровне:', e);
        }
    }, 1000);
}
